 ...
 */
export function analyzeSpecificityPatterns(text) {
...
function detectPossessivePatterns(text) {
...
function detectGenericPatterns(text) {
    const patterns = new Map();
...
function extractPatternMatches(text, regex, patternLabel, category, patterns) {
    let match;
...
function extractBarePluralPatches(text, regex, patterns) {
    let match;
    while ((match = regex.exec(text)) !== null) {
...
function extractDominantNouns(possessivePatterns) {
...
function interpretSpecificityRatio(ratio) {
    if (ratio > 0.4) {
...
function generateGuidance(ratio, dominantNouns) {
    const lines = [];
