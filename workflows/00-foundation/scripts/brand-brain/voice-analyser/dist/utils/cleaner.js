 ...
 */
export function cleanArticleContent(markdown) {
    const report = {
        isClean: true,
        artifactsRemoved: {
            base64Images: 0,
...
export function isValidWord(word) {
    ...
    if (/^https?|^www\.|^gravatar|^cdn\.|^static\./i.test(word))
        return false;
    // Filter out file extensions (as standalone "words")
    if (/^\.(jpg|jpeg|png|gif|svg|webp|js|css|html|pdf|mp4|webm)$/i.test(word))
        return false;
    if (/^(jpg|jpeg|png|gif|svg|webp|js|css|html|pdf|mp4|webm)$/i.test(word))
        return false;
    ...
    if (/^\d+$/.test(word) && !(word.length === 4 && word >= '1900' && word <= '2100'))
        return false;
    // Filter out common image dimension patterns
    if (/^\d+x\d+$/i.test(word))
        return false;
 ...
 */
export function validateCorpusQuality(articles) {
 ...
 */
export function filterVocabulary(words) {
    return words.filter(isValidWord);
}
/**
 * Get cleaning statistics for reporting
 */
export function getCleaningStats(reports) {
    const breakdown = {
        base64Images: 0,
        urlFragments: 0,
        htmlTags: 0,
